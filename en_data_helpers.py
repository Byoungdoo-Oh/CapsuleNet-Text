...
def load_data(dataset, root="./en_datasets"):
	if dataset == 'IMDB':
		return load_imdb()
...
def load_mr(pos, neg):
	positive_examples = list(open(pos, 'r', encoding='ISO-8859-1').readlines())
	positive_examples = [s.strip() for s in positive_examples]
	negative_examples = list(open(neg, 'r', encoding='ISO-8859-1').readlines())
	negative_examples = [s.strip() for s in negative_examples]
	...
	# Build vocabulary
	NUM_WORDS, documents = utils.return_numWords(x_text)
	max_document_length = max([len(x.split(' ')) for x in x_text])
	...
	# X_TRAIN = X[:int(len(X)*9/10)]
	# Y_TRAIN = Y[:int(len(Y)*9/10)]

	# X_DEV = X[int(len(X)*9/10):int(len(X)*95/100)]
	# Y_DEV = Y[int(len(Y)*9/10):int(len(Y)*95/100)]

	# X_TEST = X[int(len(X)*95/100):]
	# Y_TEST = Y[int(len(Y)*95/100):]

	# return (X_TRAIN, Y_TRAIN), (X_DEV, Y_DEV), (X_TEST, Y_TEST), len(vocab_processor.vocabulary_)+1, max_document_length
	return (X_TRAIN, Y_TRAIN), (X_TEST, Y_TEST), len(word_index)+1, max_document_length
...
def load_sst1(train, dev, test):
	...
	for line in [line.split(",", 1) for line in open(train).readlines()]:
		...
		x_train.append(clean_str_sst(line[1]))
